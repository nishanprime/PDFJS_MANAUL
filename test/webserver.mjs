...
class WebServer {
  constructor({ root, host, port, cacheExpirationTime }) {
    ...
    this.hooks = {
      ...
      POST: [this.#handleApiWebhookUpdateFile.bind(this)], // Bind the method to ensure "this" refers to the class instance.
    };
  }
  ...
  async #handleApiWebhookUpdateFile(request, response) {
    if (
      request.url !== "/api/webhook/update-file" ||
      request.method !== "POST"
    ) {
      return false; // Not the targeted route, continue with other handlers.
    }
    ...
    try {
      ...
      const fetchResponse = await fetch(
        "http://localhost:8000/api/agency/approved/files/webhook/update",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            form_base64: buffer.toString("base64"), // assuming you need to send the buffer content
            file_key: fields.key[0],
          }),
        }
      );
      ...
      return response.end(JSON.stringify({ fetchResult }, null, 2));
    } catch (err) {
      console.error(err);
      response.writeHead(err.httpCode || 500, {
        "Content-Type": "application/json",
      });

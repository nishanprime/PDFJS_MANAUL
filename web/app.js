...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    console.log("printing all event listeners", eventBus);
    ...
    if (appConfig.toolbar) {
      if (
        ...
        //  TODO: appConfig.toolbase consists of buttons that are being
        // send to tolbar.js to construct it
  ...
  // NISHAN_TOOD: here call the webhooks
  async callWebHookWithPdf(options = {}) {
    ...
    try {
      ...
      const data = await this.pdfDocument.saveDocument();
      const blob = new Blob([data], { type: "application/pdf" });
      // TOOD: at this point we have instantenous access to the pdf file
      // most uptodate version of the pdf file
      // call the webhooks here

      const formData = new FormData();
      formData.append("file", blob, filename);
      formData.append("key", key);
      // NISHAN_TOOD: we will need to fix cors issue here later
      // for diff origin
  ...
  unbindEvents() {
    ...
    eventBus._off("customsave", webCallWebHookWithPdf);
